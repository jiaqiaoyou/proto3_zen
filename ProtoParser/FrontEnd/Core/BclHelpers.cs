    ...
    public static class BclHelpers
    {
        ...
        private static void WriteTimeSpanImpl(ref ProtoWriter.State state, TimeSpan timeSpan, DateTimeKind kind)
        {
            switch (state.WireType)
            {
                ...
                case WireType.StartGroup:
                    var scaled = new PrimaryTypeProvider.ScaledTicks(timeSpan, kind);
                    state.WriteMessage<PrimaryTypeProvider.ScaledTicks>(SerializerFeatures.OptionSkipRecursionCheck, scaled, SerializerCache<PrimaryTypeProvider>.InstanceField);
                    break;
        ...
        public static TimeSpan ReadTimeSpan(ref ProtoReader.State state)
        {
            switch (state.WireType)
            {
                case WireType.String:
                case WireType.StartGroup:
                    var scaled = state.ReadMessage<PrimaryTypeProvider.ScaledTicks>(default, default, serializer: SerializerCache<PrimaryTypeProvider>.InstanceField);
